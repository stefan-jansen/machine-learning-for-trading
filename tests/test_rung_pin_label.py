...
`14_backtest` registers one mean-forecast ensemble per label. A pin on universe and family
alone therefore matches one row per label - three on 2026-09-14, at +0.566 (`fwd_ret_15m`),
+0.215 (`fwd_ret_5m`) and -0.268 (`fwd_ret_60m`) - and `ORDER BY sharpe DESC LIMIT 1` picks
among them. It picked correctly only because the featured label also held the best Sharpe. A
better `fwd_ret_5m` ensemble would have moved the rank-1 rung onto a label this case study is
not featured on, and nothing in the output would have said so.
...
def test_the_predicate_and_its_mirror_agree(case_study):
    ...
    frame = pl.DataFrame(
        {
            "label": ["fwd_ret_5m", "fwd_ret_15m", "fwd_ret_60m"],
            "family": ["ensemble"] * 3,
            "universe_filter": [pin["universe_filter"]] * 3,
            "exit_at_max_days": [None] * 3,
        }
    )
