...
def _sdf_production_kwargs() -> dict:
    """The shipped SDF preset: checkpoint_epochs are GLOBAL epoch labels."""
    return {
        "n_epochs_unc": 256,
        "n_epochs_moment": 64,
        "n_epochs_cond": 1024,
...
@pytest.mark.parametrize(
    ("reductions", "arm"),
    [({}, "canonical"), ({"max_symbols": 0}, "preview")],
)
def test_sdf_checkpoint_epochs_are_clamped_to_the_stage_budget(reductions, arm) -> None:
    """Both tiers must hand the library conditional-relative epochs within its budget.

    The published schedule labels checkpoints in GLOBAL epochs - unconditional stage
    length plus a conditional epoch - so the preset ends at 256 + 1024 = 1280.
    StochasticDiscountFactorConfig reads checkpoint_epochs as CONDITIONAL-RELATIVE and
    rejects anything above n_epochs_cond. The clamp used to be conditional on
    `reductions` being non-empty, so canonical passed 1280 through and every canonical
    SDF fit failed while every preview passed.
    """
    from types import SimpleNamespace

    from case_studies.utils.latent_factors.adapter import _resolve_model_configuration

    case = SimpleNamespace(model_kwargs={"sdf": _sdf_production_kwargs()})
    model_kwargs, _n_factors, _n_epochs, _fold_workers, _meta = _resolve_model_configuration(
        case, "sdf", {}, dict(reductions)
    )

    budget = model_kwargs["n_epochs_cond"]
    assert model_kwargs["checkpoint_epochs"] == [256, 512, 768, 1024], arm
    assert max(model_kwargs["checkpoint_epochs"]) <= budget, arm
    assert 1280 not in model_kwargs["checkpoint_epochs"], arm


def test_sdf_clamping_does_not_shorten_the_published_schedule() -> None:
    """Clamping the model config must not drop a checkpoint the schedule promises.

    The global labels are rebuilt from the clamped list, so 1280 reappears as
    n_epochs_unc + 1024. If clamping shortened the schedule the population would
    silently lose its final checkpoint.
    """
    from types import SimpleNamespace
    ...
    case = SimpleNamespace(model_kwargs={"sdf": _sdf_production_kwargs()})
    model_kwargs, _n_factors, n_epochs, _fold_workers, _meta = _resolve_model_configuration(
        case, "sdf", {}, {}
    )
    checkpoints = _expected_latent_checkpoints("sdf", n_epochs=n_epochs, model_kwargs=model_kwargs)

    assert list(checkpoints) == [256, 512, 768, 1024, 1280]
    assert checkpoints[-1] == model_kwargs["n_epochs_unc"] + max(model_kwargs["checkpoint_epochs"])
