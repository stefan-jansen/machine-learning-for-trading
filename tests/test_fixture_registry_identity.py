...
def test_declining_the_preview_tier_strips_every_preview_parameter(tmp_path) -> None:
    """A canonical run must receive no `PREVIEW_*` parameter, whatever it is called.

    `tests/generate_intermediates.py` passes `overrides["parameters"]` verbatim with
    `research_preview=False`, so any preview-only name left in reaches a notebook whose
    `EXECUTION_TIER` is still "canonical". The notebooks refuse that combination outright -
    "canonical execution cannot declare preview reductions" - so the fixture build, which
    every other job runs against, fails on the notebook's first cell.

    The strip used to name `PREVIEW_REDUCTIONS` alone, and went stale the moment
    `cme_futures` 13 through 17 declared `PREVIEW_LABELS`, `PREVIEW_MAX_PREDICTIONS` and
    `PREVIEW_MAX_BASELINE_ROWS`. This asserts the prefix rule rather than a list, so the
    next preview parameter is covered without anyone remembering to add it here.
    """
    from tests.pm_helpers import injected_parameters
    ...
    declined = injected_parameters(py_path, overrides, tmp_path, research_preview=False) or {}
    ...
    assert declined["MAX_SYMBOLS"] == 5, "a non-preview override must still reach the notebook"
