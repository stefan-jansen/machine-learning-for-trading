...
class CryptoExecutionEnv(gym.Env):
    ...
    def reset(self, seed: int | None = None, options: dict | None = None):
        ...
        # Start at random point with enough data for execution
        max_start = self.n_samples - self.horizon - 1
        self.start_idx = self.rng.integers(0, max_start)
        self.step_idx = 0
    ...
    def _trade_metrics(
        ...
        market_impact = self.impact_coefficient * (
            np.sqrt(max(participation_rate, 0.0)) + participation_rate**2
        )
