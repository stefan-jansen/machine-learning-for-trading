...
# Legacy TEST removed - use BENCHMARK_SCALE env var instead

# Benchmark scale configuration
# BENCHMARK_SCALE: XS, S, M, L, XL, XXL (or legacy: TINY, SMALL, MEDIUM, LARGE)
BENCHMARK_SCALE = os.environ.get("BENCHMARK_SCALE", "").upper()

# Chapter directory paths
from utils.paths import get_chapter_dir

CHAPTER_DIR = get_chapter_dir(8)
...
# Legacy scale configurations (for backward compatibility)
SCALE_CONFIGS = {
    "TINY": (50, 3_000, 50_000, 250_000),  # ~10MB OHLCV
    "SMALL": (100, 15_000, 500_000, 2_500_000),  # ~100MB OHLCV
    "MEDIUM": (250, 30_000, 2_000_000, 10_000_000),  # ~500MB OHLCV
    "LARGE": (500, 30_000, 5_000_000, 25_000_000),  # ~1GB OHLCV
}

# Map legacy names to new names
LEGACY_TO_NEW = {"TINY": "M", "SMALL": "L", "MEDIUM": "L", "LARGE": "XL"}
...
def get_scale_config(scale: str) -> dict:
    ...
    Args:
        scale: Scale name (XS, S, M, L, XL, XXL or legacy TINY, SMALL, MEDIUM, LARGE)

    Returns:
        Dict with ohlcv and tick configuration
    """
    # Check for new-style scale names first
    if scale in SCALE_CONFIGS_NEW:
        return SCALE_CONFIGS_NEW[scale]

    # Map legacy to new
    if scale in LEGACY_TO_NEW:
        return SCALE_CONFIGS_NEW[LEGACY_TO_NEW[scale]]

    # Default to S (equivalent to old SMALL)
    return SCALE_CONFIGS_NEW["S"]
...
if BENCHMARK_SCALE and (BENCHMARK_SCALE in SCALE_CONFIGS_NEW or BENCHMARK_SCALE in SCALE_CONFIGS):
    ACTIVE_SCALE = (
        BENCHMARK_SCALE
        if BENCHMARK_SCALE in SCALE_CONFIGS_NEW
        else LEGACY_TO_NEW.get(BENCHMARK_SCALE, "S")
    )
...
BENCHMARK_DIR = TMP_DIR  # Alias for backward compatibility
