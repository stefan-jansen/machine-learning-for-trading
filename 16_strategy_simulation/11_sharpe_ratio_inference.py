...
show_with_alt(
    fig,
    f"Histogram of {N_SIMULATIONS:,} simulated Sharpe estimates in grey with the formula's "
    "sampling distribution drawn over it as an amber curve, a dotted vertical line at zero and a "
    "dashed green line at the true Sharpe. The histogram follows the curve closely and is wide: "
    f"it spans {simulated_srs.min():.2f} to {simulated_srs.max():.2f} around a true value of "
    f"{DEMO_TRUE_SR}, and {_share_negative:.0%} of the draws come out negative even though every "
    f"one was generated by the same strategy over the same {DEMO_SAMPLE_DAYS} days.",
)
...
def _crossing(days: float) -> str:
    """Name the target Sharpe at which the curve first falls below *days*, or say it never does."""
    for target, required in zip(sr_range, min_trl_values, strict=True):
        if required <= days:
            return f"{target:.2f}"
    return "no target on this axis"


show_with_alt(
    fig,
    "Line chart on a logarithmic vertical axis of the trading days required against the target "
    "annualized Sharpe ratio, with dashed horizontal lines at one, two and three years. The curve "
    f"falls steeply, from {min_trl_values[0]:,.0f} days at a target of {sr_range[0]:.2f} to "
    f"{min_trl_values[-1]:,.0f} at {sr_range[-1]:.2f}. It comes down to three years at a target "
    f"of {_crossing(756)} and to one year only at {_crossing(252)}.",
)
