...
# returns. `GaussianHMM` floors the emission covariance at `min_covar`, and in
# native units a daily FX return's variance is orders of magnitude below that
# floor, so the fitted covariance would be set by the regularizer rather than by
# the data. On the shortest fold EM then loses positive definiteness outright and
# every restart is rejected by the stability check, leaving nothing to return.
# Scaling puts the variance above the floor and every fold converges. The cell
# below prints the two quantities the argument rests on, so the comparison is
...
print(f"GaussianHMM min_covar floor:            {GaussianHMM().min_covar:.1e}")
print(f"USD return variance, native units:      {float(_native[:, 0].var()):.3e}")
print(f"USD return variance, scaled by {HMM_SCALE:g}:     {float(usd_arr[:, 0].var()):.3e}")
hmm_results = []
hmm_params = []
unstable_hmm_fits = 0
best_model = None
