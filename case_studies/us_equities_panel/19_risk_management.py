...
if EXECUTION_TIER == "canonical":
    ...
    validation_set = study.backtests.freeze(
        ...
        # `label_artifact` is not comparable here either. 20_strategy_analysis ranks validation
        # Sharpe within one label, so a set declaring the label as varying is one it refuses.
        comparison_contract={"comparable_fields": ["cv", "feature_artifacts"]},
    )
