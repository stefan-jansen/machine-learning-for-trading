...
spans = [
    (f"Fold {s['fold']}", kind, s[f"{key}_start"], s[f"{key}_end"])
    for s in splits
    for kind, key in (("Training bars", "train"), ("Validation bars", "val"))
]
spans += [
    (f"Fold {len(splits)}", "Training bars", min(s["train_start"] for s in splits), HOLDOUT_START),
    (f"Fold {len(splits)}", "Sealed holdout", HOLDOUT_START, HOLDOUT_END),
]
span_colors = {
...
fig.add_vline(x=HOLDOUT_START, line_dash="dash", line_color=COLORS["negative"])
fig.update_layout(
    title=(
        "Every fold trains left of the validation span it is scored on"
        "<br><sup>Shaded region is the sealed holdout; the dashed rule is its start. "
        "The last row is the emitted holdout fold, whose training bars all predate "
        "the rule. Fold spans overlap because the features are built from trailing "
        "windows and the fold tag selects rows rather than changing values.</sup>"
    ),
    xaxis_title="Session",
    yaxis_title="",
    height=360,
...
for s in splits[1:]:
    fig.add_vline(x=pd.Timestamp(s["val_start"]), line_dash="dot", line_color=COLORS["neutral"])
fig.update_layout(
    title=(
        "The HAR forecast tracks the level of realized variance, not its spikes"
        "<br><sup>Cross-sectional median across symbols per session, validation rows only. "
        "Dotted rules are fold boundaries. Both series are means of squared one-minute "
        "log returns.</sup>"
    ),
    xaxis_title="Session",
...
fig.update_layout(
    title=(
        ...
        "symbol-session, on validation rows only. Whiskers omit outliers.</sup>"
    ),
    xaxis_title="Fold",
...
if n_tested > 0:
    ...
    ic_title = (
        ...
        "<br><sup>Mean cross-sectional Spearman IC on validation rows, with HAC 95% "
        "intervals. Colored bars are the features retained by Benjamini-Hochberg at 5% "
        "across those tested; neutral bars are exploratory estimates.</sup>"
    )
    fig = go.Figure(
        go.Bar(
            x=plot_ic["mean_ic"],
...
# inflation factor is the number to read first: it reports how much of the
# apparent significance was an artifact of serial correlation in the IC series.
# A factor near one on an overlapping-return panel is a signal that the series
# was not in time order, which is why the series here comes from
# `cross_sectional_ic_series` rather than from a `group_by`.
#
...
#    factor near one on overlapping returns.
#
# ### Known Limitations
#
