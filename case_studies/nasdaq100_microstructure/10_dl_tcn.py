...
# %% [markdown]
# # TCN — NASDAQ-100 Microstructure
#
# TCN supplies the convolutional benchmark in the NASDAQ sequence. Dilated
# causal convolutions should capture multi-scale intraday dynamics if they are
# present in the microstructure features.
#
# **Learning Objectives**:
# - Test whether temporal convolutions add value over linear, GBM, NLinear, and LSTM baselines
# - Compare convolutional receptive fields against recurrent memory on the same label
# - Prepare a clean architecture comparison before PatchTST
#
# **Book Reference**: Chapter 13
#
# **Prerequisites**: [`06_linear`](06_linear.ipynb), [`07_gbm`](07_gbm.ipynb), [`08_dl_nlinear`](08_dl_nlinear.ipynb), [`09_dl_lstm`](09_dl_lstm.ipynb)

# %%
"""TCN — nasdaq100_microstructure deep learning."""
...
_baselines = load_best_ic_per_family(
    ["linear", "gbm", "deep_learning"], case_studies=[CASE_STUDY_ID]
)
if not _baselines.is_empty():
    for row in _baselines.iter_rows(named=True):
        ...
        elif row["family"] == "deep_learning" and row["config_name"] == "nlinear":
            prior_baselines["NLinear (Ch13.1)"] = row["ic_mean"]
        elif row["family"] == "deep_learning" and row["config_name"].startswith("lstm"):
            prior_baselines["LSTM (Ch13.2)"] = row["ic_mean"]
...
if prior_baselines:
    ...
    print(
        "  No prior results found — run 06_linear.py, 07_gbm.py, 08_dl_nlinear.py, and 09_dl_lstm.py first"
    )
...
# ## 7. Key Takeaways
#
# TCN now has a clean provenance chain in the NASDAQ sequence. Any gap versus
# NLinear, LSTM, or PatchTST will now reflect the architecture rather than a
# notebook that secretly trained multiple models.
