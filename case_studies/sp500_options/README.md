...
**Validation performance.** Validation Sharpe is -0.2083 [-1.7870, +1.5297] over 462 daily periods, with maximum drawdown -0.8549 driven by a single 232-day episode into June 2020. Across the eight validation folds the Sharpe ranges [-1.272, +1.282] with a standard deviation of 1.806, so the point estimate is not distinguishable from fold-to-fold noise. Every one of the 1,632 development backtests on this label is negative. The 1,572 equal-weight baselines have mean Sharpe -1.105, median -1.108 and P90 -0.884, and the best of them is -0.311 (`gbm / default_mae`); the 60 allocation-stage rows run from -0.873 to the carrier's -0.208, which is the best anywhere on the label. The carrier's own 60-variant allocation cohort deflates to DSR_ER -0.0250 (p=0.759). Linear PBO has only two combinations, below the ten-combination reporting threshold.
