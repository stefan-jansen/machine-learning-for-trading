...
#
# The guard matters more than the population. Both computations are idempotent
# and reproduce the stored values exactly, but running them still rewrites the
# registry file, and the registry is what every published number in this case
# study is read from. So they run only when a table is missing or empty. Against
# a populated registry this notebook writes nothing and every cell below is a
# read.
