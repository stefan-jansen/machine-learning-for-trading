...
if MAX_SYMBOLS > 0:
    # `top_entities` is the one rule for reducing a panel's entity axis, and the reason to
    # call it rather than write the reduction here is its second sort key. Row counts tie
    # readily on a monthly firm panel, and a tie broken by frame order is not stable between
    # this notebook and the model notebooks that reduce the same panel later.
    kept = top_entities(eval_panel, MAX_SYMBOLS, ENTITY_COL)
    eval_panel = eval_panel.filter(pl.col(ENTITY_COL).is_in(pl.Series(ENTITY_COL, kept).implode()))
