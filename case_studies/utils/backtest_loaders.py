"""Price and configuration loading for the Ch16-19 backtesting notebooks.

Handles schema normalization across all 9 case studies.

Predictions do NOT come from here. A backtest names its members through a
``CandidateSet`` and runs them through ``run_backtests`` on a ``Study``, which reads the
prediction catalog. ``load_backtest_predictions`` was this module's prediction entry
point and had no caller anywhere in the repository; it was removed rather than kept as a
second, unexercised route, because the exclusions it applied read as enforced while
reaching nothing.
